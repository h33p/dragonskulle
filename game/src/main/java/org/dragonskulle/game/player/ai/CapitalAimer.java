...
public class CapitalAimer extends AiPlayer {
	...
	private void findCapital() {
		...
		for (int nodeNumber: nodesInGraph) {
			...
			if (node.getHexTile().get().getClaimant() == opponentPlayer && node.getHexTile().get().getBuilding().isCapital()) {
				oppCapNode = node;
			}
			else if (node.getHexTile().get().getClaimant() == mPlayer.get() && node.getHexTile().get().getBuilding().isCapital()) {
				capNode = node;
			}
		}
	}
